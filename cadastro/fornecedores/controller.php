  ...
  $acao = (isset($_REQUEST['acao'])&& $_REQUEST['acao'] !=NULL)?$_REQUEST['acao']:'';
	if($acao == 'inserir')
	{
			 ...
			 //$sql = "SELECT * FROM usuarios WHERE usuario='".$_POST['usuario']."' AND senha='".$_POST['senha']."'";
			 	$sql='INSERT INTO cadastrofornecedores(DataCadastro,Nome,Cpf,Rg,cep,endereco,bairro,cidade,numero,CNPJ,RazaoSocial,NomeFantasia,InscEstadual,Telefone,Celular,Email) 
				   VALUES ("'.$DataCadastro.'","'.$nome.'","'.$Cpf.'","'.$cep.'","'.$endereco.'", "'.$bairro.'","'.$cidade.'","'.$numero.'","'.$CNPJ.'","'.$RazaoSocial.'","'.$NomeFantasia.'","'.$InscricaoEstadual.'","'.$telefone.'","'.$celular.'","'.$email.'")';
				//echo $sql;
	...
	if($acao == 'alterar')
	{
			 ...
			 $sql = "UPDATE cadastrofornecedores set DataCadastro='".$DataCadastro."',Nome='".$nome."',Cpf='".$Cpf."',
			 cep='".$cep."',endereco='".$endereco."',bairro='".$bairro."',numero='".$numero."',
			 CNPJ='".$CNPJ."',RazaoSocial='".$RazaoSocial."',NomeFantasia='".$NomeFantasia."',InscEstadual='".$InscricaoEstadual."',
			 telefone='".$telefone."',celular='".$celular."',email='".$email."' where CodigoFornecedor= '".$codigo."' ";
			 $resultados =mysqli_query($conexao,$sql)or die (mysql_error());
	...
	if($acao == 'consultar')
	{
		...
		function inverteData($data){    
		 ...
		 $sql = "select * from cadastrofornecedores;";
		 $resultados = mysqli_query($conexao,$sql)or die (mysql_error());
	    ...
	    if(mysqli_num_rows($resultados) <= 0)
		 ...
		 $total = 0;
			...
			<tr>
                ...
                data-id="<?php echo $row['CodigoFornecedor'];?>" data-nome="<?php echo $row['Nome'];?>"
                data-cep="<?php echo $row['Cep'];?>" data-cpf="<?php echo $row['Cpf'];?>"
                ...
                data-id="<?php echo $row['CodigoFornecedor'];?>"e class="btn btn-danger"><i class="glyphicon glyphicon-trash"> Excluir</i></button>
                  </td>
	...
	if($acao == 'excluir')
	{
			 ...
			 $sql = "delete FROM cadastrofornecedores where CodigoClientes= '$codigo' ";
			 $resultados = mysqli_query($conexao,$sql)or die (mysql_error());
	...
	if($acao == 'filtrar')
	{
	    ...
	    if($nome!="")
		{
		 ...
		 $sql = "SELECT * FROM cadastrofornecedores WHERE Nome ='".$_POST["nome"]."'";
		 $resultados = mysqli_query($conexao,$sql)or die (mysql_error());
		 echo $sql;
...
if($acao == 'filtrarnome')
	{
		$nome = $_POST['nome'];	
		 $sql = "select * from cadastrofornecedores WHERE Nome LIKE '%".$_POST["nome"]."%';";
		 $resultados = mysqli_query($conexao,$sql)or die (mysql_error());
		echo $sql;
		 ...
		 if(mysqli_num_rows($resultados) <= 0)
		{
		  ...
		  else
		  {
			...
			<tr>
                ...
                <td>	
                <td><button type="button"  data-toggle="modal"  data-target="#altera_modal" class="btn btn-info" data-action="shred" 
                data-id="<?php echo $row['CodigoFornecedor'];?>" 
                data-cep="<?php echo $row['Cep'];?>" data-cpf="<?php echo $row['Cpf'];?>"
                data-CNPJ="<?php echo $row['CNPJ'];?>" data-RazaoSocial="<?php echo $row['RazaoSocial'];?>" data-inscricaoEstadual="<?php echo $row['InscEstadual'];?>"
                data-endereco="<?php echo $row['Endereco'];?>" data-bairro="<?php echo $row['Bairro'];?>" data-cidade="<?php echo $row['Cidade'];?>"
                data-numero="<?php echo $row['Numero'];?>" data-telefone="<?php echo $row['Telefone'];?>" data-celular="<?php echo $row['Celular'];?>" data-email="<?php echo $row['Email'];?>">
